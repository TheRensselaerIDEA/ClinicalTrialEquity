...
sun_plot_process<-function(function_name,subgroup_detail,function_real,cut1,cut2,cut3,cut4,sig_cut,alpha_subgroup,beta_subgroup,whether_equitable = '',seg_length,ylimit = TRUE){
  ...
  if (whether_equitable == "Equitable(p)"){
    ...
    text(beta_subgroup+0.05,1.5, "Patient",cex=1.2, col = "chocolate4")
  }
  ...
  legend("bottomright",inset=0.01, title="Inequity Levels",
         ...
         fill=c("#696969","#000000","#b2182b","#ef8a62","#FDAE61","#f7f7f7","#ABD9E9","#253494"), horiz=FALSE, cex=1)
  
}



segments_calculation_2<-function(l1,l2,u1,u2,given_p,given_participation, function_real,seg_length){
  x<- seq(seg_length,1-seg_length,seg_length)
  find_l1 <- TRUE
  find_l2 <- TRUE
  find_u1 <- TRUE
  find_u2 <- TRUE
  ...
  for (i in x){
    if (function_real(given_p, i, given_participation,TRUE) <=l1 & find_l1 ){
      print(paste0("l1 = ", i))
      a[1]<-i
      find_l1 = FALSE
    }
    else if (function_real(given_p, i, given_participation,TRUE) <=l2& find_l2& !find_l1){
      print(paste0("l2 = ", i))
      find_l2 = FALSE
      a[2]<-i
    }
    else if (function_real(given_p, i, given_participation,TRUE) >= u1& find_u1 & !find_l1& !find_l2){
      print(paste0("u1 = ", i))
      find_u1 = FALSE
      a[3]<-i
    }
    else if (function_real(given_p, i,given_participation, TRUE) >= u2 & find_u2 & !find_l1& !find_l2& !find_u1){
      print(paste0("u2 = ", i))
      find_u2 = FALSE
      a[4]<-i
    }
...
sun_plot_process_2<-function(function_name,subgroup_detail,function_real,cut1,cut2,cut3,cut4,sig_cut,alpha_subgroup,beta_subgroup,given_participation,whether_equitable = '',seg_length){
  
  x_subgroup <- seq(seg_length,1-seg_length,seg_length)
  
  segs<-segments_calculation_2(cut1,cut2,cut3,cut4,alpha_subgroup,given_participation,function_real,seg_length)
  print(segs)
  y_subgroup<-function_real(alpha_subgroup,x_subgroup,given_participation,TRUE)
  
  ylim(0,0.0006)
  plot(x_subgroup,y_subgroup,
       main=paste0(function_name, " for ", subgroup_detail),
       ylab="\u03C4 = Metric Value",xlab ="Observed rate",
       type="l",col="black",lwd = 1.5,ylim=c(0,0.0006))
  ...
  if (whether_equitable == "Equitable(p)"){
    abline(v = x_subgroup[round((x_vert2+x_vert3)/2)],col= "#878080", lwd=3, lty=2)
    text(x_subgroup[x_vert4]+0.05,0.0001, "Patient",cex=2, col = "#878080" )
  }
  else{
    abline(v = beta_subgroup,col= "chocolate4", lwd=3, lty=2)
    text(beta_subgroup+0.05,0.0001, "Patient",cex=1.2, col = "chocolate4")
  }
  
  abline(v = alpha_subgroup,col= "green4", lwd=3, lty=2)
  text(alpha_subgroup+0.05,0, "Ideal",cex=1.2, col = "green4")
  
  legend("bottomright", inset=.02, title="Inequity Levels",
         c("Subgroup Absent in NHANES",
           "Subgroup Absent in NHANES & RCT",
           "Subgroup Absent in RCT", 
           paste0("Equitable (",0,"\u2264 \u03C4<",round(cut3,5)," or p>",round(sig_cut,2),")"), 
           paste0("Dependent (",round(cut3,5),"\u2264 \u03C4<",round(cut4,5),")"),
           paste0("Highly Dependent (",round(cut4,5),"\u2264\u03C4)")), 
         fill=c("#696969","#000000","#b2182b","#f7f7f7","#FDAE61","#ef8a62"), horiz=FALSE, cex=1)

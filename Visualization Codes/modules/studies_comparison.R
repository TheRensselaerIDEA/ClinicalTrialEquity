make_table_combine<-function(background_info,RCT_info,background_info_lab,x, sig_t, cutoff1,cutoff2) {
  ...
  merged_df$NMIEquityValue<- mapply(Normalized_Mutual_Information,as.numeric(merged_df$Background_Rate),as.numeric(merged_df$Observed_Rate),as.numeric(merged_df$participant_rate), for_plot = FALSE)

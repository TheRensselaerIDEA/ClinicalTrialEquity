...
######################################
ui <- dashboardPage(skin = "black", title = "RCT Equity Visualization Supplement",
                    dashboardHeader(title = tags$div(
                      class = "title-text",
                      tags$div(id = "logo_block", tags$img(src="Rensselaer.png", id="header_logo"))
                               #,HTML("<p style='display: inline; font-size: 1.2em; vertical-align: middle;'>Trial<b><span style = 'color: #990000;'><span style='padding-left:2px; padding-right: 2px;'>Equity</span></span></b></p>")
                    ...
                    dashboardSidebar(
                      ...
                      sidebarMenu(
                        ...
                        menuItem("Equity Analysis",tabName = "main"),
                        uiOutput("taskInputs"),
                        ...
                        strong("Color Reference for Inequity Levels"),
         ...
         $(document).ready(function() {
            ...
            $("header").find("nav").append(\'<div class="title-text title-lside">RCT Equity <b><span style = "color: #990000;"><span style="padding-left:2px; padding-right: 2px;">Visualization Supplement</span></span></b></div>\');
...
server <- function(input, output, session) {
  ...
  userType <- reactiveVal("researcher")
  
  # Tracks the type of task 
  taskType <- reactiveVal("physician_comparison_distribution")
  ...
  output$trialProfiler <- renderUI({
    if (showBackgroundUploader()) {
      showModal(modalDialog(
        tags$div(
          class = "start-modal",
          HTML("<h1 class = 'start-modal'>Trial<b><span style = 'color: #990000;'><span style='padding-left:5px; padding-right: 5px;'>Equity</span></span></b></h1>"),
          div(class="start-modal-copy",
          h2('Evaluate inequities of clinical trials.\n', class="start-modal"),
          h2('Compare inequities among studies.\n', class="start-modal"),
      ...
      column(12,
             tags$div(
               ...
               selectInput("diseaseType", "Select the Disease Type of Your Trial Data:",
      ...
      column(12,
             ...
             tags$div(
               ...
               radioButtons(
                 ...
                 selected = "sample",
                 inline = TRUE,
                 choiceNames = c("Use a sample clinical trial dataset"),
                 choiceValues = c("sample")
               ),
               tags$div(
                   
                 hidden(selectInput("trialSampler", label = "Pick a sample clinical trial code file", selected = "ACCORD for type-II diabetes", multiple = FALSE, choices = c("ACCORD for type-II diabetes"))%>%
                          shinyInput_label_embed(
                            shiny_iconlink() %>%
                              bs_embed_popover(
                                title = "Sample RCT for Diabetes", content = "Please select the ACCORD study. For multiple-RCT example, please select HYPERTENSION for the disease type in the previous page. ", placement = "left"
                              )
                          )),

                 selectInput("trialSampler2", label = "Pick sample clinical trial code files", selected = "SPRINT for hypertension", multiple = TRUE, choices = c("SPRINT for hypertension","ALLHAT for hypertension" ))%>%
                          shinyInput_label_embed(
                            shiny_iconlink() %>%
                              bs_embed_popover(
             ...
             tags$div(class = "footer",
                      HTML('<a href="https://github.com/TheRensselaerIDEA/ClinicalTrialEquity" target = "_blank">About</a>  |  <a href="https://info.rpi.edu/web-privacy-statement">Privacy Policy</a>  |  '),#<a href="">Clinical Trial Data Template</a>
                      downloadLink("rctTemplate5", "Clinical Trial Data Template")
             )
      ...
      column(12, class = "side_sub",
             ...
             selectInput(inputId = "tasks_researcher", label = "Select a task",
                         c("Study vs Target Population" = "distribution_p",
                           "Equity Evaluation" = "metric_r",
                           "Comparative Study of Equity" = "comparison_r"),
      ...
      column(12, class = "side_sub",
         selectInput(inputId = "equity_metrics", label = "Select an Equity Metric",
           ...
           shinyInput_label_embed(
             shiny_iconlink() %>%
               bs_embed_popover(
                 title = "Equity Metrics", content = "Equity metrics are functions of disease-specific rates of protected subgroups in the RCT. 
                 Log Disparate Impact ensures that the subgroup assignment rates in RCT is similar to those in the matched target population.
                 Adjusted Equal Opportunity ensures that the proportions of people assigned in an ideal RCT but with different protected attribute 
                 values should be assigned into the real clinical trial with equal rates.
                 Quality Metric assesses the shift in RCT patients’ representations of their disease.", placement = "left"
               )
           ),
         use_bs_popover(),
  ...
  output$mainResult <- renderUI({
    if (taskType() == "physician_comparison_distribution"){
      ...
      column(12,
             tags$div(
               ...
               tags$div(
                ...
                h2("Equity Levels for Subgroups"),
      ...
      column(12,
             tags$div(
               style="padding-bottom: 50px;background-color: #ffffff;padding-left: 15px;padding-right: 15px;padding-top: 50px;margin-bottom: 20px;",
               h2("Equity of Demographic Characterisitcs in the RCTs"),
               uiOutput("study_comparison_demo")
             ),
             tags$div(
               style="padding-bottom: 50px;background-color: #ffffff;padding-left: 15px;padding-right: 15px;padding-top: 50px;margin-bottom: 20px;",
               h2("Equity of Clinical Characterisitcs in the RCTs"),
               uiOutput("study_comparison_clinical")
             )
      )
    }
  ...
  observeEvent(input$equity_metrics, {
    ...
    else if (identical(input$equity_metrics, "QM_metric")) {metricType("QM")}
  })
  ...
  output$rctTemplate1 <- downloadHandler (
    filename <- function() {
      "ACCORD.csv"
    },
    content <- function(file) {
      file.copy("./ACCORD.csv", file)
    },
    contentType = "text/csv"
  )
  output$rctTemplate2 <- downloadHandler (
    filename <- function() {
      "ACCORD.csv"
    },
    content <- function(file) {
  ...
  output$rctTemplate3 <- downloadHandler (
    filename <- function() {
      "ACCORD.csv"
    },
    content <- function(file) {
  ...
  output$rctTemplate4 <- downloadHandler (
    filename <- function() {
      "ACCORD.csv"
    },
    content <- function(file) {
  ...
  df_file<- reactive({
    if (diseaseType() == "hypertension"){
      ...
      path_of_files<-list()
      i2<-1
      if ("SPRINT for hypertension" %in% input$trialSampler2){
        name_list[i2]<- "SPRINT"
        path_of_files[i2]<- "./SPRINT.csv"
        i2<-i2+1
      }
      if ("ALLHAT for hypertension" %in% input$trialSampler2){
        name_list[i2]<-"ALLHAT"
  ...
  df_file_background<- reactive({
    if (diseaseType() == "hypertension"){
      ...
      name_list[1]<-"nhanes_hypertension.csv"
      name_list[2]<-"nhanes_hypertension_lab.csv"
      name_list[3]<-"nhanes_hypertension_combined.csv"
      
      path_of_files[1]<-"./modules/background_data/nhanes_hypertension.csv"
      path_of_files[2]<-"./modules/background_data/nhanes_hypertension_lab.csv"
      path_of_files[3]<-"./modules/background_data/nhanes_hypertension_combined.csv"
      ...
      name_list[1]<-"nhanes_diabetes.csv"
      name_list[2]<-"nhanes_diabetes_lab.csv"
      name_list[3]<-"nhanes_diabetes_combined.csv"
      
      path_of_files[1]<-"./modules/background_data/nhanes_diabetes.csv"
      path_of_files[2]<-"./modules/background_data/nhanes_diabetes_lab.csv"
      path_of_files[3]<-"./modules/background_data/nhanes_diabetes_combined.csv"
   ...
   output$user_selected_file_researcher <- renderUI({
     ...
     if(taskType() == "researcher_comparison_trial"){
       column(12, class = "side_sub",
              selectInput("upload_this_file_researcher_compare", "Choose file(s) for comparative anlysis:", files_names, multiple = TRUE)
       )
     }
     else{
       column(12, class = "side_sub",
              selectInput("upload_this_file_researcher", "Choose a file for analysis:",files_names)
       )
     }   }) 
  ...
  output$user_var_display <- renderUI({
    ...
    selectInput("vars_display_sundb", "Choose Variables for Equity Evaluation", 
                choices  = colnames,
                #selected = colnames[1],
                multiple= TRUE
                )
  ...
  df_generate_sunburst <- reactive({
    ...
    else if (metricType() == "QM"){
      df_processed <-generate_sunburst_df(background_data,as.list(input$vars_sundb_order),user_data,significanceThreshold(),-log(1-lowerThreshold()),-log(1-upperThreshold()),Quality_Metric, FALSE)
  ...
  output$sun_plot_di <-renderPlot({
    ...
    if (is.null(returned_info_click)){
      ...
      if (metricType() == "LDI"){
        sun_plot_process("Log Disparate Impact",returned_info_click$ids,Log_Disparate_Impact,log(1-upperThreshold()) ,log(1-lowerThreshold()),
  ...
  output$sunburst_text <- renderUI({
    ...
    if (is.null(returned_info_click)){
      ...
      if (metricType() == "LDI"){
        metric_used<- "Log Disparate Impact"
      }
      else if (metricType() == "AEO"){
        metric_used<- "Adjusted Equal Opportunity"
      }
      else if (metricType() == "QM"){
        metric_used<- "Clinical Quality Metric"
      }
    }
    ...
    if ( returned_info_click$EquityLable=="Equitable(p)"){
    ...
    else if (returned_info_click$EquityLable=="Equitable"){
    ...
    HTML(paste(
      ...
      "Subgroup Name :  ", returned_info_click$ids, "<br>",
      "Its equity value of  ", returned_info_click$ids, " using the metric ", metric_used, " is ",  returned_info_click$EquityValue,
  ...
  output$study_comparison_demo <- renderUI({
  ...
  output$study_comparison_clinical <- renderUI({
    req(input$upload_this_file_researcher_compare)
    df_list<-df_upload_files() 
